...
# GaussDB/PostgreSQL/libpq 连接类 SQLSTATE。
# 适配点：真实 GaussDB 连接通过 psycopg2/libpq 暴露错误。连接断开或服务端拒绝
# 连接时，不应该依赖英文错误文本；优先用 SQLSTATE 判断，文本只作为驱动没有提供
# pgcode 时的兜底。08xxx 是 connection exception 类，57P0x 是服务端关闭/暂不可用。
...
# 55P03 FOR UPDATE NOWAIT 行锁不可用。它们都表示当前事务/语句因并发竞争失败，
# 在 RAGFlow 的“整段数据库操作重试”场景下可以重试。
PSYCOPG_RETRYABLE_TRANSACTION_SQLSTATES = {"40P01", "40001", "55P03"}

# DDL 幂等判断使用的 SQLSTATE。
# 适配点：迁移代码需要区分“已经迁移过，可以跳过”和真正失败。真实 GaussDB
# 实测重复列是 42701，重复表/索引名是 42P07，缺失索引/约束对象是 42704。
...
def _iter_exception_chain(exc: BaseException) -> Iterable[BaseException | object]:
    """遍历 Peewee 包装异常、异常链和 args 中携带的原始驱动异常。"""
    seen: set[int] = set()
...
def sqlstate_from_exception(exc: BaseException) -> str | None:
    """从 psycopg2 原始异常或 Peewee 包装异常中提取 SQLSTATE。"""
