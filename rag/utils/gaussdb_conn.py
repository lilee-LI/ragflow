    ...
    def _build_set_clause(self, new_value: dict, is_meta: bool, condition: dict) -> tuple[str, list[Any]]:
        ...
        def append_jsonb_update(column: str) -> None:
            ...
            if "remove" in operations:
                ...
                remove_expr = (
                    f"json_remove({column}, json_unquote(json_search({column}, 'one', "
                    "jsonb_array_element_text(%s::jsonb, 0))))::jsonb"
                )
                remove_param = json.dumps([escaped_item], ensure_ascii=False)
            if "add" in operations:
                base_expr = (
                    f"COALESCE({remove_expr}, {column}, '[]'::jsonb)"
                    if remove_expr is not None
                    else f"COALESCE({column}, '[]'::jsonb)"
                )
                expression = (
                    "(SELECT CASE WHEN candidate @> %s::jsonb THEN candidate "
                    "ELSE json_array_append(candidate, '$', %s::jsonb)::jsonb END "
                    f"FROM (SELECT {base_expr} AS candidate) AS jsonb_update)"
                )
                params.extend(
                    [
