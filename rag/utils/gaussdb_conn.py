...
def _jsonb_array_remove_expr(source: str) -> str:
    return f"COALESCE((SELECT array_to_json(array_agg(value))::jsonb FROM (SELECT jsonb_array_elements_text({source}) AS value) AS existing WHERE value <> %s), '[]'::jsonb)"


def _jsonb_array_add_expr(source: str) -> str:
    return f"COALESCE((SELECT array_to_json(array_agg(value))::jsonb FROM (SELECT jsonb_array_elements_text({source}) AS value UNION SELECT %s AS value) AS combined), '[]'::jsonb)"


def _is_undefined_table_error(exc: Exception) -> bool:
    return (
        getattr(exc, "pgcode", None) == _UNDEFINED_TABLE_SQLSTATE
        or getattr(exc, "sqlstate", None) == _UNDEFINED_TABLE_SQLSTATE
    )
...
def _infer_chunk_vector_size(documents: list[dict]) -> int:
    dimensions = {
        ...
        if (match := VECTOR_COLUMN_RE.fullmatch(str(column)))
    }
    if len(dimensions) != 1:
        raise ValueError(
            "cannot infer a single GaussDB vector dimension from chunk documents"
        )
...
class GaussDBConnection(GaussDBConnectionBase):
    ...
    def create_idx(self, index_name: str, dataset_id: str, vector_size: int, parser_id: str = None):
        # Commit the table, regular indexes and vector columns first. A failed
        # gsdiskann build must not roll the functional table back with it, but
        # its error is still propagated to prevent silent vector degradation.
        statements = [
    ...
    def insert(self, documents: list[dict], index_name: str, knowledgebase_id: str = None) -> list[str]:
        ...
        try:
            ...
            try:
                ...
                if (
                    is_meta
                    or not _is_undefined_table_error(exc)
                    or self.index_exist(index_name, knowledgebase_id)
                ):
                    raise
    ...
    def sql(self, sql: str, fetch_size: int = 128, format: str = "json"):
        ...
        validator = GaussDBSQLValidator.readonly_guard(default_limit=fetch_size)
        validated = validator.validate_and_patch(sql)
        qualified_sql = validator.qualify_runtime_tables(validated.sql, self.schema)
        rows, description = self._fetch_all_with_description(qualified_sql, [])
    ...
    def _build_doc_meta_upsert(
        ...
        sql = f"INSERT INTO {table} (id, kb_id, meta_fields) VALUES (%s, %s, %s::jsonb) ON DUPLICATE KEY UPDATE kb_id = VALUES(kb_id), meta_fields = VALUES(meta_fields)"
        return sql, params
    ...
    def _parse_match_expressions(
        ...
        if gaussdb_search_params:
            tokenized_keywords = gaussdb_search_params.get("keywords")
            if isinstance(tokenized_keywords, (list, tuple)):
                keywords = [str(keyword).strip() for keyword in tokenized_keywords if str(keyword).strip()]
    ...
    def _scoped_search_condition(self, condition: dict | None, knowledgebase_ids: list[str] | None) -> dict:
        ...
        kb_ids = normalize_kb_ids(knowledgebase_ids)
        if kb_ids and "kb_id" not in effective:
        ...
        for key, value in new_value.items():
            if key == "remove":
                ...
                for column, item in value.items():
                    ...
                    jsonb_operations.setdefault(column, {})["remove"] = str(item)
                continue
            if key == "add":
                if not isinstance(value, dict):
                ...
                for column, item in value.items():
                    ...
                    jsonb_operations.setdefault(column, {})["add"] = str(item)
                continue
            if key == "metadata" and not is_meta and isinstance(value, dict):
                ...
                continue
            if not is_meta and (vector_match := VECTOR_COLUMN_RE.fullmatch(str(key))):
                dim = self.ddl.validate_vector_dim(int(vector_match.group("dim")))
                fragments.append(f"{key} = %s::floatvector({dim})")
                params.append(vector_literal(value, dim))
                fragments.append(f"{self.ddl.vector_valid_column_name(dim)} = %s")
                params.append(True)
                continue
            if not is_meta and (valid_match := VECTOR_VALID_COLUMN_RE.fullmatch(str(key))):
            ...
            if key in jsonb_columns:
                ...
                fragments.append(f"{key} = %s")
                params.append(value)
        for column, operations in jsonb_operations.items():
            expression = f"COALESCE({column}, '[]'::jsonb)"
            if "remove" in operations:
                expression = _jsonb_array_remove_expr(expression)
                params.append(operations["remove"])
            if "add" in operations:
                expression = _jsonb_array_add_expr(expression)
                params.append(operations["add"])
    ...
    def _build_where_clause(self, condition: dict, knowledgebase_id: str | None, is_meta: bool) -> tuple[str, list[Any]]:
        ...
        for key, value in effective.items():
            if key == "exists":
                ...
                expression = filter_column_expr(value, is_meta)
                if is_dynamic_chunk_field(value, is_meta):
            ...
            if key == "must_not" and isinstance(value, dict) and "exists" in value:
                ...
                expression = filter_column_expr(value["exists"], is_meta)
                if is_dynamic_chunk_field(value["exists"], is_meta):
            ...
            expression = filter_column_expr(key, is_meta)
            if is_dynamic_chunk_field(key, is_meta):
...
def filter_column_expr(column: str, is_meta: bool = False) -> str:
    ...
    if is_dynamic_chunk_field(column, is_meta):
