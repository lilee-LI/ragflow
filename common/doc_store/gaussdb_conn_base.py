...
from common.doc_store.gaussdb_conn_pool import GAUSSDB_CONN, GaussDBConnectionError, GaussDBConnectionPool
...
class GaussDBSQLValidator:
    ...
    def qualify_runtime_tables(self, sql: str, schema: str) -> str:
        """Qualify validated DocEngine base tables for distributed execution.

        A distributed CN can push a SELECT to DNs without carrying the
        session's search_path. Runtime SQL is validated while still
        unqualified, then this method adds the configured schema only to real
        base tables. CTE references remain untouched.
        """
        if not re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]{0,62}", schema or ""):
            raise UnsafeGaussDBSQL("invalid runtime schema")
    ...
    def _validate_jsonb_paths(self, ast) -> None:
        ...
        json_classes = tuple(cls for cls in (getattr(exp, "JSONExtractScalar", None), getattr(exp, "JSONExtract", None)) if cls)
        jsonb_classes = tuple(cls for cls in (getattr(exp, "JSONBExtractScalar", None), getattr(exp, "JSONBExtract", None)) if cls)
...
class GaussDBDDLBuilder:
    ...
    def __init__(self, schema: str, distributed: bool = False):
    ...
    def qualified_name(self, table: str) -> str:
        return f"{self.schema}.{self.validate_identifier(table)}"
    ...
    def index_name(self, table: str, suffix: str) -> str:
        ...
        if len(name) <= self.MAX_IDENTIFIER_LENGTH:
            return name
        digest = hashlib.sha1(name.encode("ascii")).hexdigest()[:10]
  ...
  PRIMARY KEY (kb_id, id)
    ...
    def build_doc_meta_table_ddls(self, meta_table: str) -> list[str]:
        ...
        idx = self.index_name(meta_table, "kb_id")
        return [
  ...
  PRIMARY KEY (id)
    ...
    def build_regular_index_ddls(self, table: str) -> list[str]:
        name = self.qualified_name(table)
        return [f"CREATE INDEX IF NOT EXISTS {self.index_name(table, column)} ON {name} ({column})" for column in self.REGULAR_INDEX_COLUMNS]

    def build_fulltext_ugin_ddl(self, table: str) -> str:
        name = self.qualified_name(table)
        idx = self.index_name(table, "fts_all")
  ...
  USING ugin(to_tsvector('simple', {expression}))"""
    ...
    def build_ustore_check_sql(self, table: str) -> tuple[str, list[str]]:
    ...
    def validate_vector_dim(self, dim: int) -> int:
        ...
        if self.distributed and value > 1024:
            raise ValueError("Distributed GaussDB vector dimensions above 1024 are not supported")
        if value > 4096:
...
class GaussDBSearchBuilder:
    ...
    def __init__(self, schema: str, distributed: bool = False):
        self.ddl = GaussDBDDLBuilder(schema=schema, distributed=distributed)
    ...
    def build_condition_where(self, condition: dict | None) -> tuple[str, list[Any]]:
        ...
        for key, value in (condition or {}).items():
            ...
            if self._is_dynamic_column(column):
                ...
                continue
            column = self._storage_column(column)
    ...
    def build_text_score_expr(self, keywords: list[str]) -> tuple[str, list[Any]]:
        fts_keywords, cjk_keywords = self._partition_text_keywords(keywords)
        score_parts: list[str] = []
        params: list[Any] = []
        if fts_keywords:
            query_text = self._text_query_param(fts_keywords)
            score_parts.append(
                "(" + " + ".join(f"{weight} * COALESCE(ts_rank(to_tsvector('simple', coalesce({column}, ' ')), plainto_tsquery('simple', %s)), 0)" for column, weight in self.FTS_WEIGHTS.items()) + ")"
            )
            params.extend([query_text] * len(self.FTS_WEIGHTS))
        for keyword in cjk_keywords:
            score_parts.append("(" + " + ".join(f"{weight} * CASE WHEN strpos(COALESCE({column}, ''), %s) > 0 THEN 1.0 ELSE 0.0 END" for column, weight in self.FTS_WEIGHTS.items()) + ")")
            params.extend([keyword] * len(self.FTS_WEIGHTS))
        return "(" + " + ".join(score_parts or ["0.0"]) + ")", params
    ...
    def build_highlight_expr(self, field_name: str, keywords: list[str]) -> tuple[str, list[Any]]:
        ...
        return (
            f"ts_headline('simple', COALESCE({expression}, ' '), plainto_tsquery('simple', %s)) AS _highlight",
    ...
    def build_aggregation_sql(
        ...
        if field in self.JSONB_ARRAY_AGG_COLUMNS:
            sql = (
                ...
                "FROM ("
                f"SELECT jsonb_array_elements_text(COALESCE({field}, '[]'::jsonb)) AS value "
                f"FROM {table_name} WHERE {where_clause}"
                ") AS expanded "
                "GROUP BY value ORDER BY count DESC, value ASC LIMIT %s"
            )
        else:
            sql = f"SELECT {field_expression} AS value, COUNT(1) AS count FROM {table_name} WHERE {where_clause} AND {field_expression} IS NOT NULL GROUP BY value ORDER BY count DESC, value ASC LIMIT %s"
        return sql, [*where_params, int(limit)]

    def build_position_order_sql(self) -> str:
        return "COALESCE((page_num_int #>> '{0}')::int, 0) ASC, COALESCE((position_int #>> '{0,3}')::int, 0) ASC, COALESCE((top_int #>> '{0}')::int, 0) ASC"
    ...
    def _build_text_match_expr(self, keywords: list[str]) -> tuple[str, list[Any]]:
        ...
        params: list[Any] = []
        if fts_keywords:
            match_parts.append(f"{self.build_fts_vector_expr()} @@ plainto_tsquery('simple', %s)")
            params.append(self._text_query_param(fts_keywords))
        for keyword in cjk_keywords:
            match_parts.append("(" + " OR ".join(f"strpos(COALESCE({column}, ''), %s) > 0" for column in self.FTS_WEIGHTS) + ")")
            params.extend([keyword] * len(self.FTS_WEIGHTS))
        return "(" + " AND ".join(match_parts or ["TRUE"]) + ")", params

    def _partition_text_keywords(self, keywords: list[str]) -> tuple[list[str], list[str]]:
        fts_keywords = []
        cjk_keywords = []
        for keyword in keywords:
            target = cjk_keywords if self.CJK_RE.search(keyword) else fts_keywords
            target.append(keyword)
        return fts_keywords, cjk_keywords
    ...
    def _is_dynamic_column(self, column: str) -> bool:
        return not (column in self.CHUNK_COLUMNS or column == "row_id()" or self.VECTOR_COLUMN_RE.fullmatch(column) or self.VECTOR_VALID_COLUMN_RE.fullmatch(column))
    ...
    def _column_expr(
        ...
        *,
        ...
        if self._is_dynamic_column(column):
            return extra_field_expr(column, prefix=prefix, text=text)
        storage_column = self._storage_column(column)
...
class GaussDBConnectionBase(DocStoreConnection):
    def __init__(self, pool: GaussDBConnectionPool | None = None, logger_name: str = "ragflow.gaussdb_conn"):
        ...
        # 适配点：DocEngine adapter 和 Memory Store adapter 语义不同，但生产
        # 环境应共享同一个懒加载连接池入口。单测仍可通过 pool 参数注入 fake
        # pool，避免初始化真实 GaussDB 连接。
        self.pool = pool or GAUSSDB_CONN
