...
def _ensure_model_provider_ready_via_api(base_url: str, auth_header: str) -> dict:
    ...
    if not current_llm:
        raise RuntimeError("The seeded tenant has no default chat model.")
    if not current_embd:
        raise RuntimeError("The seeded tenant has no default embedding model.")

    _, defaults_payload = _api_request_json(_build_url(base_url, "/api/v1/models/default"), headers=headers)
    default_models = _response_data(defaults_payload).get("models") or []
    enabled_defaults = {str(model.get("model_type") or ""): model for model in default_models if isinstance(model, dict) and model.get("enable") is True}
    chat_default = enabled_defaults.get("chat")
    embedding_default = enabled_defaults.get("embedding")
    if not chat_default:
        raise RuntimeError(f"The current model API returned no enabled default chat model: {default_models}")
    if not embedding_default:
        raise RuntimeError(f"The current model API returned no enabled default embedding model: {default_models}")

    expected_chat_name = current_llm.rsplit("@", 2)[0]
    expected_embedding_name = current_embd.rsplit("@", 2)[0]
    if chat_default.get("model_name") != expected_chat_name:
        raise RuntimeError(f"Default chat model does not match tenant llm_id: {chat_default}")
    if embedding_default.get("model_name") != expected_embedding_name:
        raise RuntimeError(f"Default embedding model does not match tenant embd_id: {embedding_default}")

    return {
        "tenant_id": tenant_id,
        "has_provider": True,
        "created_provider": False,
        "normalized_defaults": False,
        "llm_factories": sorted({str(model.get("model_provider") or "") for model in enabled_defaults.values()}),
    }
