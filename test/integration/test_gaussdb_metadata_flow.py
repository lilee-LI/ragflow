...
async def test_tc_mgf_808_metadata_filter_pushdown_full_chain(
    ...
    assert sql_filter == (
        "(lower(meta_fields #>> '{status}') = %s OR "
        "jsonb_exists(meta_fields #> '{status}', %s))"
    )
...
def test_tc_wrt_310_metadata_upsert_failure_keeps_existing_row_in_live_gaussdb(
    gaussdb_env, register_table, gaussdb_admin_conn
):
    from api.db.services.doc_metadata_service import DocMetadataService
    ...
    with gaussdb_admin_conn.cursor() as cur:
        cur.execute(
            sql.SQL(
                ...
                "CHECK (COALESCE(meta_fields #>> '{{status}}', '') <> 'forbidden')"
            ).format(
                sql.Identifier(gaussdb_env["schema"], meta_table),
