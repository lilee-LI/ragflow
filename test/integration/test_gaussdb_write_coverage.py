...
def test_tc_wrt_011_create_idx_advisory_lock_serializes_live_creates(gaussdb_env, table_name, gaussdb_admin_conn):
    ...
    assert reusable.get("post-concurrency", table, ["kb-wrt-011"])["content_with_weight"] == ("connection pool remains writable")
    assert _fetch_one(
        gaussdb_admin_conn,
        sql.SQL("SELECT content_with_weight, q_1024_vec_valid FROM {} WHERE kb_id = %s AND id = %s").format(_qualified(gaussdb_env["schema"], table)),
...
def test_tc_wrt_102_insert_writes_vector_and_default_state(gaussdb_env, table_name, gaussdb_admin_conn):
    ...
    assert conn.insert([{"id": "c1", "kb_id": kb_id, "q_1024_vec": [0.1] * 1024, "content_with_weight": "x"}], table, kb_id) == []
    row = _fetch_one(
        gaussdb_admin_conn,
        sql.SQL("SELECT id, kb_id, q_1024_vec_valid, available_int, removed_kwd FROM {} WHERE id = %s AND kb_id = %s").format(_qualified(gaussdb_env["schema"], table)),
        ["c1", kb_id],
    )
...
def test_tc_wrt_103_insert_aligns_heterogeneous_batch_columns(gaussdb_env, table_name, gaussdb_admin_conn):
    ...
    rows = _fetch_all(
        gaussdb_admin_conn,
        sql.SQL("SELECT id, q_1024_vec, q_1024_vec_valid, important_kwd FROM {} WHERE kb_id = %s ORDER BY id").format(_qualified(gaussdb_env["schema"], table)),
        [kb_id],
    )
...
def test_tc_wrt_104_insert_upsert_overwrites_same_kb_chunk(gaussdb_env, table_name, gaussdb_admin_conn):
    ...
    row = _fetch_one(
        gaussdb_admin_conn,
        sql.SQL("SELECT content_with_weight, docnm_kwd, q_1024_vec_valid FROM {} WHERE id = %s AND kb_id = %s").format(_qualified(gaussdb_env["schema"], table)),
        ["c1", kb_id],
    )
...
def test_tc_wrt_105_insert_keeps_same_chunk_id_across_kbs(gaussdb_env, table_name, gaussdb_admin_conn):
    ...
    rows = _fetch_all(
        gaussdb_admin_conn,
        sql.SQL("SELECT id, kb_id, content_with_weight, q_1024_vec_valid FROM {} WHERE id = %s ORDER BY kb_id").format(_qualified(gaussdb_env["schema"], table)),
        ["c1"],
    )
    assert rows == [
...
def test_tc_wrt_106_insert_extracts_metadata_and_infers_batch_dimension(gaussdb_env, table_name, gaussdb_admin_conn):
    ...
    row = _fetch_one(
        gaussdb_admin_conn,
        sql.SQL("SELECT group_id, docnm_kwd, metadata, q_1024_vec_valid FROM {} WHERE id = %s AND kb_id = %s").format(_qualified(gaussdb_env["schema"], table)),
        ["c1", kb_id],
    )
...
def test_tc_wrt_108_insert_normalizes_single_kb_id_list_to_scalar(gaussdb_env, table_name, gaussdb_admin_conn):
    ...
    assert _fetch_one(
        gaussdb_admin_conn,
        sql.SQL("SELECT id, kb_id, content_with_weight, q_1024_vec_valid FROM {} WHERE id = %s AND kb_id = %s").format(_qualified(gaussdb_env["schema"], table)),
        ["c1", kb_id],
    ) == ("c1", kb_id, "test", True)
...
def test_tc_wrt_112_insert_rejects_mismatched_vector_dimension_without_row(gaussdb_env, table_name, gaussdb_admin_conn):
    ...
    result = conn.insert(
        [
            {"id": "valid", "kb_id": kb_id, "q_1024_vec": [0.2] * 1024},
            {"id": "invalid", "kb_id": kb_id, "q_1024_vec": invalid_vector},
        ],
        table,
        kb_id,
    )
    ...
    assert (
        _fetch_one(
            gaussdb_admin_conn,
            sql.SQL("SELECT COUNT(*) FROM {} WHERE kb_id = %s AND id IN (%s, %s)").format(_qualified(gaussdb_env["schema"], table)),
            [kb_id, "valid", "invalid"],
        )[0]
        == 0
    )
...
def test_tc_wrt_202_insert_rejects_placeholder_when_dimension_cannot_be_inferred(gaussdb_env, table_name, gaussdb_admin_conn):
    ...
    with gaussdb_admin_conn.cursor() as cur:
        cur.execute(
            sql.SQL("CREATE TABLE {} (id VARCHAR(256) NOT NULL, kb_id VARCHAR(256) NOT NULL, content_with_weight TEXT, PRIMARY KEY (kb_id, id))").format(_qualified(gaussdb_env["schema"], table))
        )
...
def test_tc_wrt_405_get_rejects_cross_kb_duplicate_chunk_id(gaussdb_env, table_name, gaussdb_admin_conn):
    ...
    assert _fetch_all(
        gaussdb_admin_conn,
        sql.SQL("SELECT id, kb_id FROM {} WHERE id = %s ORDER BY kb_id").format(_qualified(gaussdb_env["schema"], table)),
        ["c1"],
    ) == [("c1", "k1"), ("c1", "k2")]
...
def test_tc_wrt_407_get_decodes_jsonb_and_vector_values(gaussdb_env, table_name, gaussdb_admin_conn):
    ...
    stored = _fetch_one(
        gaussdb_admin_conn,
        sql.SQL("SELECT id, kb_id, doc_id, content_with_weight, chunk_data, extra, metadata, q_1024_vec, q_1024_vec_valid FROM {} WHERE id = %s AND kb_id = %s").format(
...
def test_tc_wrt_408_search_orders_page_position_and_top(gaussdb_env, table_name, gaussdb_admin_conn):
    ...
    with gaussdb_admin_conn.cursor() as cur:
        cur.execute(
            sql.SQL(
                "CREATE TABLE {} (id VARCHAR(256) NOT NULL, kb_id VARCHAR(256) NOT NULL, page_num_int JSONB, position_int JSONB, top_int JSONB, content_with_weight TEXT, PRIMARY KEY (kb_id, id))"
    ...
    with gaussdb_admin_conn.cursor() as cur:
        ...
        rows = cur.fetchall()
    assert order_clause == ("COALESCE((page_num_int #>> '{0}')::int, 100000000) ASC, COALESCE((position_int #>> '{0,3}')::int, 100000000) ASC, COALESCE((top_int #>> '{0}')::int, 100000000) ASC")
...
def test_tc_wrt_503_update_changes_scoped_normal_column(gaussdb_env, table_name, gaussdb_admin_conn):
    ...
    assert _fetch_all(
        gaussdb_admin_conn,
        sql.SQL("SELECT kb_id, available_int FROM {} WHERE id = %s ORDER BY kb_id").format(_qualified(gaussdb_env["schema"], table)),
        ["c1"],
    ) == [("k1", 0), ("k2", 1)]
    ...
    assert _fetch_one(
        gaussdb_admin_conn,
        sql.SQL("SELECT available_int, doc_id FROM {} WHERE id = %s AND kb_id = %s").format(_qualified(gaussdb_env["schema"], table)),
        ["c1", "k1"],
    ) == (0, None)
...
def test_tc_wrt_505_update_remove_string_sets_jsonb_column_null(gaussdb_env, table_name, gaussdb_admin_conn):
    ...
    assert _fetch_one(
        gaussdb_admin_conn,
        sql.SQL("SELECT important_kwd, content_with_weight FROM {} WHERE id = %s AND kb_id = %s").format(_qualified(gaussdb_env["schema"], table)),
        ["c1", "k1"],
    ) == (None, "test")
...
def test_tc_wrt_612_delete_doc_id_compensates_cancelled_task(gaussdb_env, table_name, gaussdb_admin_conn):
    ...
    try:
        with gaussdb_admin_conn.cursor() as cur:
            cur.execute(
                sql.SQL("CREATE FUNCTION {}.{}() RETURNS trigger AS $body$ BEGIN RAISE EXCEPTION 'forced integration delete failure'; END; $body$ LANGUAGE plpgsql").format(
        ...
        assert _fetch_all(
            gaussdb_admin_conn,
            sql.SQL("SELECT id, kb_id, doc_id, content_with_weight FROM {} ORDER BY kb_id, id").format(_qualified(gaussdb_env["schema"], table)),
            [],
        ) == [
...
def test_tc_wrt_703_delete_idx_metadata_does_not_drop_chunk_table(gaussdb_env, table_name, register_table, gaussdb_admin_conn):
    ...
    with gaussdb_admin_conn.cursor() as cur:
        cur.execute(
            sql.SQL("CREATE TABLE {} (id VARCHAR(256) NOT NULL, kb_id VARCHAR(256) NOT NULL, content_with_weight TEXT, PRIMARY KEY (kb_id, id))").format(_qualified(gaussdb_env["schema"], chunk_table))
        )
        cur.execute(sql.SQL("CREATE TABLE {} (id VARCHAR(256) NOT NULL, kb_id VARCHAR(256) NOT NULL, meta_fields JSONB, PRIMARY KEY (id))").format(_qualified(gaussdb_env["schema"], meta_table)))
    gaussdb_admin_conn.commit()
    _insert_rows(
        gaussdb_admin_conn,
        gaussdb_env["schema"],
    ...
    assert _fetch_one(
        gaussdb_admin_conn,
        sql.SQL("SELECT id, kb_id, content_with_weight FROM {} WHERE id = %s").format(_qualified(gaussdb_env["schema"], chunk_table)),
        ["c1"],
    ) == ("c1", "k1", "chunk canary")


def test_tc_wrt_704_delete_multiple_kbs_uses_single_in_and_keeps_table(gaussdb_env, table_name, gaussdb_admin_conn):
    from rag.utils.gaussdb_conn import GaussDBConnection
    ...
    with gaussdb_admin_conn.cursor() as cur:
        ...
        cur.executemany(
            sql.SQL("INSERT INTO {} (id, kb_id, content_with_weight) VALUES (%s, %s, %s)").format(_qualified(gaussdb_env["schema"], table)),
...
def test_tc_wrt_705_index_exist_is_true_for_created_table(gaussdb_env, table_name, gaussdb_admin_conn):
    ...
    with gaussdb_admin_conn.cursor() as cur:
        cur.execute(sql.SQL("CREATE TABLE {} (id VARCHAR(256) NOT NULL, kb_id VARCHAR(256) NOT NULL, PRIMARY KEY (kb_id, id))").format(_qualified(gaussdb_env["schema"], table)))
    gaussdb_admin_conn.commit()
...
def test_tc_wrt_707_index_exist_is_read_only_and_idempotent(gaussdb_env, table_name, gaussdb_admin_conn):
    ...
    gaussdb_admin_conn.commit()
    catalog_sql = "SELECT column_name, data_type, is_nullable, column_default FROM information_schema.columns WHERE table_schema = %s AND table_name = %s ORDER BY ordinal_position"
...
def test_tc_wrt_709_delete_idx_missing_table_is_idempotent_and_pool_recovers(gaussdb_env, table_name, gaussdb_admin_conn):
    ...
    try:
        ...
        assert _fetch_one(
            gaussdb_admin_conn,
            sql.SQL("SELECT id, kb_id, content_with_weight FROM {} WHERE id = %s AND kb_id = %s").format(_qualified(schema, table)),
            ["recovery", "kb-wrt-709"],
...
def test_tc_wrt_1005_lifecycle_rebuilds_same_doc_with_new_content(gaussdb_env, table_name, gaussdb_admin_conn):
    ...
    with gaussdb_admin_conn.cursor() as cur:
        cur.execute(
            sql.SQL("INSERT INTO {} (id, kb_id, doc_id, content_with_weight, content_ltks, q_4_vec, q_4_vec_valid) VALUES (%s, %s, %s, %s, %s, %s::floatvector(4), TRUE)").format(
                _qualified(gaussdb_env["schema"], table)
            ),
    ...
    assert [(row["id"], row["doc_id"], row["content_with_weight"], row["content_ltks"]) for row in result.chunks] == [("c1", "d1", "new", "new")]
    ...
    rows = _fetch_all(
        gaussdb_admin_conn,
        sql.SQL("SELECT id, kb_id, doc_id, content_with_weight, content_ltks, q_4_vec, q_4_vec_valid FROM {} WHERE id = %s AND kb_id = %s").format(_qualified(gaussdb_env["schema"], table)),
        ["c1", kb_id],
    )
    assert len(rows) == 1
...
def test_tc_wrt_1008_lifecycle_duplicate_upsert_keeps_one_latest_row(gaussdb_env, table_name, gaussdb_admin_conn):
    ...
    rows = _fetch_all(
        gaussdb_admin_conn,
        sql.SQL("SELECT content_with_weight, docnm_kwd, q_1024_vec, q_1024_vec_valid FROM {} WHERE id = %s AND kb_id = %s").format(_qualified(gaussdb_env["schema"], table)),
        ["c1", kb_id],
    )
    assert len(rows) == 1
...
def test_tc_wrt_1009_lifecycle_keeps_cross_kb_same_id_isolated(gaussdb_env, table_name, gaussdb_admin_conn):
    ...
    assert _fetch_all(
        gaussdb_admin_conn,
        sql.SQL("SELECT id, kb_id, content_with_weight FROM {} WHERE id = %s ORDER BY kb_id").format(_qualified(gaussdb_env["schema"], table)),
        ["c1"],
    ...
    assert _fetch_one(
        gaussdb_admin_conn,
        sql.SQL("SELECT content_with_weight, q_1024_vec_valid FROM {} WHERE id = %s AND kb_id = %s").format(_qualified(gaussdb_env["schema"], other_tenant_table)),
        ["c1", kb_a],
