...
async def test_tc_ret_906_dealer_search_fulltext_vector_and_hybrid_full_chain(
    ...
    with gaussdb_admin_conn.cursor() as cur:
        cur.executemany(
            sql.SQL(
                "INSERT INTO {} (id, kb_id, doc_id, title_tks, content_with_weight, content_ltks, "
                "content_sm_ltks, q_4_vec, q_4_vec_valid) "
                "VALUES (%s, %s, %s, %s, %s, %s, %s, %s::floatvector(4), %s)"
    ...
    with gaussdb_admin_conn.cursor() as cur:
        cur.execute(
            sql.SQL("SELECT id, q_4_vec_valid FROM {} ORDER BY id").format(
...
def test_tc_ret_907_search_filter_and_aggregation_remain_available_during_batch_writes(
    ...
    with ThreadPoolExecutor(max_workers=1) as pool:
        ...
        try:
            ...
            with gaussdb_admin_conn.cursor() as cur:
                cur.execute(
                    sql.SQL(
                        "SELECT id, doc_id FROM {} WHERE kb_id = %s AND doc_id = %s ORDER BY id"
                    ).format(sql.Identifier(gaussdb_env["schema"], table)),
                    [kb_id, "doc-batch"],
                )
    ...
    with gaussdb_admin_conn.cursor() as cur:
        cur.execute(
            sql.SQL("SELECT id, doc_id FROM {} WHERE kb_id = %s ORDER BY id").format(
                sql.Identifier(gaussdb_env["schema"], table)
            ),
            [kb_id],
        )
        ...
        cur.execute(
            sql.SQL("SELECT doc_id, COUNT(*) FROM {} WHERE kb_id = %s GROUP BY doc_id ORDER BY doc_id").format(
                sql.Identifier(gaussdb_env["schema"], table)
            ),
            [kb_id],
        )
...
async def test_tc_ret_1002_real_sql_execution_failure_falls_back_to_real_retrieval(
    ...
    with gaussdb_admin_conn.cursor() as cur:
        ...
        cur.execute(
            sql.SQL("ALTER TABLE {} DROP COLUMN chunk_data").format(
                sql.Identifier(gaussdb_env["schema"], table)
            )
        )
    gaussdb_admin_conn.commit()
    with gaussdb_admin_conn.cursor() as cur:
        cur.execute(
            sql.SQL(
                "SELECT id, doc_id, content_with_weight, q_4_vec_valid FROM {} WHERE kb_id = %s"
            ).format(sql.Identifier(gaussdb_env["schema"], table)),
            [kb_id],
        )
    ...
    assert [(chunk["chunk_id"], chunk["doc_id"], chunk["kb_id"]) for chunk in events[-1]["reference"]["chunks"]] == [
        ("chunk-ret-1002", doc_id, kb_id)
    ]
    assert events[-1]["reference"]["doc_aggs"] == [
        {"doc_id": doc_id, "doc_name": "fallback.txt", "count": 1}
    ]
