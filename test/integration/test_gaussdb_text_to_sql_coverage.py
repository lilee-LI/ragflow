...
def test_tc_sql_006_table_chunk_persists_real_field_map(
    ...
    assert conn.sql(
        f"SELECT chunk_data #>> '{{{amount_path}}}' AS amount FROM {table} WHERE kb_id = '{kb_id}'"
    ) == {"columns": [{"name": "amount", "type": "text"}], "rows": [["100"]]}


def test_tc_sql_007_table_chunk_data_reaches_real_jsonb_table(
    gaussdb_env, ragflow_kb_context, monkeypatch, table_name, gaussdb_admin_conn
):
    from api.db.services.knowledgebase_service import KnowledgebaseService
    from common import settings
    from rag.app.table import chunk as table_chunk
...
def test_tc_sql_411_jsonb_missing_null_and_empty_values(gaussdb_env, table_name):
    ...
    null_rows = conn.sql(
        f"SELECT doc_id FROM {table} WHERE (chunk_data #>> '{{status}}') IS NULL "
        f"AND kb_id = '{KB_ID}' ORDER BY doc_id LIMIT 128"
    )
    not_null_rows = conn.sql(
        f"SELECT doc_id FROM {table} WHERE (chunk_data #>> '{{status}}') IS NOT NULL "
        f"AND kb_id = '{KB_ID}' ORDER BY doc_id LIMIT 128"
    )
...
def test_tc_sql_501_sql_uses_real_validator_and_returns_rows(gaussdb_env, table_name):
    ...
    assert conn.sql(
        f"SELECT doc_id FROM {other_tenant_table} WHERE kb_id = '{KB_ID}'"
    )["rows"] == [["other-tenant-secret"]]
    ...
    with pytest.raises(UnsafeGaussDBSQL, match=rf"table {re.escape(other_tenant_table)} is not allowed"):
        tenant_validator.validate_and_patch(
            f"SELECT doc_id FROM {other_tenant_table} WHERE kb_id = '{KB_ID}'"
        )
    assert conn.sql(
        f"SELECT doc_id FROM {other_tenant_table} WHERE kb_id = '{KB_ID}'"
...
def test_tc_sql_504_sql_statement_timeout_is_reset_after_checkout(
    ...
    assert after == before
    assert conn.sql(
        f"SELECT doc_id FROM {table} WHERE kb_id = '{KB_ID}' AND doc_id = 'd000'"
    )["rows"] == [["d000"]]
...
async def test_tc_sql_509_adapter_database_error_and_use_sql_retry_are_real(gaussdb_env, monkeypatch):
    ...
    _create_and_insert(conn, table, KB_ID, [_row("c1", KB_ID, "d1", "doc.xlsx", 100)])
    invalid_sql = (
        f"SELECT CAST(chunk_data #>> '{{status}}' AS INTEGER) AS amount, doc_id, docnm_kwd "
        f"FROM {table} WHERE kb_id = '{KB_ID}'"
    )
    with pytest.raises(psycopg2.Error) as exc_info:
...
async def test_tc_sql_701_aggregate_sql_fetches_real_source_chunks(gaussdb_env, monkeypatch):
    ...
    sql = (
        f"SELECT SUM(CAST(chunk_data #>> '{{amount}}' AS DOUBLE PRECISION)) AS total FROM {table} "
        "WHERE chunk_data #>> '{status}' = 'active' LIMIT 3"
    )
    result, dealer = await _aggregate_result(
        ...
        [
            ...
            _row("c4", second_kb_id, "d3", "other.xlsx", 999),
        ],
        sql,
        recording=True,
    ...
    assert scoped_source.endswith("LIMIT 3")

    multi_kb_aggregate = (
        f"SELECT SUM(CAST(chunk_data #>> '{{amount}}' AS DOUBLE PRECISION)) AS total FROM {table} "
        f"WHERE kb_id IN ('{KB_ID}', '{second_kb_id}')"
    )
    ...
    assert sorted(multi_kb_rows) == [
        ...
        ["d1", "sales.xlsx", KB_ID],
        ["d2", "sales.xlsx", KB_ID],
        ["d3", "other.xlsx", second_kb_id],
    ]

    having_aggregate = (
        f"SELECT SUM(CAST(chunk_data #>> '{{amount}}' AS DOUBLE PRECISION)) AS total FROM {table} "
        "HAVING SUM(CAST(chunk_data #>> '{amount}' AS DOUBLE PRECISION)) > 0"
    )
...
async def test_tc_sql_1006_async_chat_falls_back_to_real_dealer_retrieval(
    ...
    try:
        ...
        with caplog.at_level(logging.DEBUG):
            ...
            with gaussdb_admin_conn.cursor() as cur:
                cur.execute(
                    sql.SQL("SELECT COUNT(*) FROM {} WHERE kb_id = %s AND doc_id = %s").format(
                        sql.Identifier(gaussdb_env["schema"], table)
                    ),
                    [kb_id, doc_id],
                )
