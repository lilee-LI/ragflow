...
def _require_gaussdb_schema():
    schema = (os.getenv("GAUSSDB_SCHEMA") or "").strip()
    if not schema:
        pytest.fail("set GAUSSDB_SCHEMA to a pre-created integration schema")
    return schema
...
def gaussdb_variant_preflight(record_testsuite_property):
    ...
    assert variant in {"centralized", "distributed"}, "GAUSSDB_VARIANT must be centralized or distributed for live integration"
    ...
    assert expected_dims == required_dims, f"{variant.capitalize()} integration requires GAUSSDB_EXPECTED_VECTOR_DIMS={','.join(map(str, required_dims))}"
    schema = _require_gaussdb_schema()
