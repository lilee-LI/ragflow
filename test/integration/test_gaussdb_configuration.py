...
def test_tc_cfg_612_status_reports_real_alive_payload(gaussdb_env, ragflow_http_auth):
    ...
    expected_uri = f"{os.environ['GAUSSDB_USER']}@{os.environ['GAUSSDB_HOST']}:{os.environ.get('GAUSSDB_PORT', '19995')}/{evidence['database']}?schema={gaussdb_env['schema']}"
...
def test_tc_cfg_903_create_idx_smoke_uses_truncated_real_index_names(gaussdb_env, table_name, register_table, gaussdb_admin_conn):
    ...
    try:
        ...
        assert conn.ddl.index_name(fixed_chunk_table, "fts_all") == ("idx_gdb_ragflow_it_0123456789abcdef0123456789abcdef__d839bbe2f5")
        assert conn.ddl.index_name(fixed_chunk_table, "fts_all_ngram") == ("idx_gdb_ragflow_it_0123456789abcdef0123456789abcdef__39df30fec5")
        assert conn.ddl.index_name(fixed_chunk_table, "q_1024_vec_diskann") == ("idx_gdb_ragflow_it_0123456789abcdef0123456789abcdef__945c83acf7")
        assert conn.ddl.index_name(fixed_meta_table, "kb_id") == ("idx_gdb_ragflow_doc_meta_0123456789abcdef0123456789a_b39d0ef468")

        simple_ugin_indexes = [(name, definition) for name, definition in chunk_indexes if " using ugin " in definition and "to_tsvector('simple'" in definition]
        ngram_ugin_indexes = [(name, definition) for name, definition in chunk_indexes if " using ugin " in definition and "to_tsvector('ngram'" in definition]
        diskann_indexes = [(name, definition) for name, definition in chunk_indexes if " using gsdiskann " in definition]
        meta_kb_indexes = [(name, definition) for name, definition in meta_indexes if " using ubtree " in definition and "(kb_id)" in definition]
...
def test_tc_cfg_904_health_does_not_create_objects_before_real_create_idx(gaussdb_env, table_name, register_table, gaussdb_admin_conn):
    from common.doc_store.gaussdb_conn_pool import GaussDBConnectionPool, load_gaussdb_config
    from rag.utils.gaussdb_conn import GaussDBConnection
