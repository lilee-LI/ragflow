...
def _insert(scope, rows, kb_id=None):
    ...
    values = [
        (row["id"], target_kb_id, json.dumps(row["meta_fields"], ensure_ascii=False, separators=(",", ":")))
        for row in rows
    ]
    with scope["admin_conn"].cursor() as cur:
        cur.executemany(
            sql.SQL("INSERT INTO {} (id, kb_id, meta_fields) VALUES (%s, %s, %s::jsonb)").format(
                sql.Identifier(scope["schema"], scope["table"])
            ),
            values,
        )
    ...
    with scope["admin_conn"].cursor() as cur:
        cur.execute(
            sql.SQL("SELECT id, kb_id, meta_fields FROM {} ORDER BY id").format(
                sql.Identifier(scope["schema"], scope["table"])
            )
        )
        stored = cur.fetchall()
...
def _match(scope, filters, kb_ids=None, limit=10000, logic="and"):
    ...
    result = DocMetadataService.filter_doc_ids_by_meta_pushdown(
        kb_ids or scope["kb_ids"], filters, logic, limit=limit
    )
...
def test_tc_mgf_803_greater_than_filters_numeric_values_and_rejects_non_numeric(metadata_scope):
    ...
    greater_sql = (
        "(meta_fields #>> '{amount}' ~ '^-?[0-9]+(\\.[0-9]+)?$' AND "
        "(meta_fields #>> '{amount}')::DOUBLE PRECISION > %s)"
    )
...
def test_tc_mgf_809_limit_plus_one_probe_returns_none_or_complete_result(metadata_scope, monkeypatch):
    ...
    _insert(
        metadata_scope,
        [
            {"id": doc_id, "meta_fields": {"status": "active"}}
            for doc_id in matching_ids
        ]
        + [{"id": "inactive", "meta_fields": {"status": "inactive"}}],
    )
    ...
    over_limit = DocMetadataService.filter_doc_ids_by_meta_pushdown(
        metadata_scope["kb_ids"], [{"key": "status", "op": "=", "value": "active"}], "and", limit=3
    )
    ...
    fetch_calls.clear()
    exact_limit = DocMetadataService.filter_doc_ids_by_meta_pushdown(
        metadata_scope["kb_ids"], [{"key": "status", "op": "=", "value": "active"}], "and", limit=5
    )
    ...
    fetch_calls.clear()
    under_limit = DocMetadataService.filter_doc_ids_by_meta_pushdown(
        metadata_scope["kb_ids"], [{"key": "status", "op": "=", "value": "active"}], "and", limit=6
    )
