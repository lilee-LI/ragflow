    ...
    try:
        with connection.cursor() as cur:
            for row in rows:
                cur.execute(
                    sql.SQL(
                        "INSERT INTO {}.{} (id, kb_id, doc_id, docnm_kwd, content_ltks, chunk_data, q_4_vec, q_4_vec_valid) VALUES (%s, %s, %s, %s, %s, %s::jsonb, %s::floatvector(4), TRUE)"
        ...
        connection.commit()
        with connection.cursor() as cur:
            cur.execute(sql.SQL("SELECT id, kb_id FROM {}.{} ORDER BY id").format(sql.Identifier(schema), sql.Identifier(table)))
...
async def test_tc_sql_801_text_to_sql_multikb_reference_completes_kb_id_in_live_gaussdb(
    ...
    chat = FakeSQLChatModel(f"SELECT doc_id, docnm_kwd, chunk_data #>> '{{amount}}' AS amount FROM {table} ORDER BY doc_id")
    result = await dialog_service.use_sql(
        "show all amounts",
        {"amount": "amount"},
        tenant_id,
