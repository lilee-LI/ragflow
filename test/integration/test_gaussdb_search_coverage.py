...
def _table_rows(admin_conn, schema: str, table: str) -> list[tuple[str, str]]:
    with admin_conn.cursor() as cur:
        cur.execute(sql.SQL("SELECT id, content_with_weight FROM {} ORDER BY id").format(sql.Identifier(schema, table)))
...
def _explain_ugin_probe(admin_conn, schema: str, table: str, *, config: str, keyword: str) -> str:
    ...
    try:
        ...
        with admin_conn.cursor() as cur:
            ...
            cur.execute(
                sql.SQL(f"EXPLAIN SELECT id FROM {{}} WHERE to_tsvector('{config}', {expression}) @@ plainto_tsquery('{config}', %s)").format(sql.Identifier(schema, table)),
                [keyword],
            )
        ...
        with _independent_connection() as admin_conn:
            with admin_conn.cursor() as cur:
                for table, prefix, count, content in seeds:
                    statement = sql.SQL("INSERT INTO {} (id, kb_id, content_with_weight) VALUES %s").format(sql.Identifier(conn.schema, table))
                    execute_values(
                        cur,
            ...
            with admin_conn.cursor() as cur:
                for table, prefix, count, _ in seeds:
                    cur.execute(
                        sql.SQL("SELECT COUNT(*), MIN(id), MAX(id) FROM {} WHERE kb_id = %s").format(sql.Identifier(conn.schema, table)),
                        [kb_id],
                    )
...
def test_tc_ret_109_dual_ugin_supports_chinese_english_mixed_and_inplace_upgrade(
    ...
    try:
        ...
        with gaussdb_admin_conn.cursor() as cur:
            cur.execute(
                sql.SQL("SELECT id, q_1024_vec_valid, q_1024_vec <+> %s::floatvector(1024) AS distance FROM {} WHERE kb_id = %s AND id IN (%s, %s) ORDER BY distance, id").format(
...
def test_tc_ret_105_fulltext_field_weights_affect_score(gaussdb_env, table_name):
    ...
    try:
        ...
        for field, weight in (
            ...
            assert (f"{weight} * COALESCE(ts_rank(to_tsvector('simple', coalesce({field}, ' ')), plainto_tsquery('simple', %s)), 0)") in search_sql
...
def test_tc_ret_203_invalid_vector_valid_flag_is_not_recalled(gaussdb_env, table_name, gaussdb_admin_conn):
    ...
    try:
        ...
        with gaussdb_admin_conn.cursor() as cur:
            cur.execute(
                sql.SQL("SELECT q_4_vec_valid FROM {} WHERE kb_id = %s AND id = %s").format(sql.Identifier(gaussdb_env["schema"], table)),
                [kb_id, "placeholder"],
            )
...
def test_tc_ret_210_pagerank_changes_vector_score(gaussdb_env, table_name):
    ...
    try:
        _create_chunks(
            conn, table, kb_id, [_base_row("near", kb_id, "near", q_4_vec=[1.0, 0.0, 0.0, 0.0], pagerank_fea=0), _base_row("ranked", kb_id, "ranked", q_4_vec=[0.9, 0.1, 0.0, 0.0], pagerank_fea=100)]
        )
        search_sql, params = conn._search_builder().build_search_sql(
            table=table,
            select_fields=["id"],
        ...
        result = _search(
            conn, table, kb_id, fields=["id", "_score"], matches=[MatchDenseExpr("q_4_vec", [1.0, 0.0, 0.0, 0.0], "float", "cosine", 10, {"similarity": 0.0})], rank={constants.PAGERANK_FLD: 1}
        )
        scores = conn.get_scores(result)
...
def test_tc_ret_302_hybrid_scores_are_normalized_and_positive(gaussdb_env, table_name):
    ...
    try:
        _create_chunks(
            ...
            kb_id,
            [
                _base_row("text", kb_id, "alpha", title_tks="alpha"),
                _base_row("vector", kb_id, "beta", q_4_vec=[1.0, 0.0, 0.0, 0.0]),
                _base_row("both", kb_id, "alpha", title_tks="alpha", q_4_vec=[0.9, 0.1, 0.0, 0.0]),
            ],
        )
...
def test_tc_ret_306_hybrid_threshold_filters_final_score(gaussdb_env, table_name):
    ...
    try:
        _create_chunks(
            ...
            table,
            kb_id,
            [_base_row("hit", kb_id, "alpha", title_tks="alpha", q_4_vec=[1.0, 0.0, 0.0, 0.0]), _base_row("below", kb_id, "alpha", content_ltks="alpha", q_4_vec=[0.0, 1.0, 0.0, 0.0])],
        )
...
def test_tc_ret_603_jsonb_array_aggregation(gaussdb_env, table_name):
    from rag.utils.gaussdb_conn import GaussDBConnection
    ...
    try:
        ...
        result = _search(conn, table, kb_id, agg=["tag_kwd"])
...
def test_tc_ret_604_aggregation_value_count_rows_are_used(gaussdb_env, table_name):
    ...
    try:
        _create_chunks(
            conn, table, kb_id, [_base_row("a", kb_id, "one", doc_type_kwd="active"), _base_row("b", kb_id, "two", doc_type_kwd="active"), _base_row("c", kb_id, "three", doc_type_kwd="inactive")]
        )
        result = _search(conn, table, kb_id, fields=["id"], agg=["doc_type_kwd"])
...
def test_tc_ret_704_tag_features_stay_out_of_gaussdb_sql_score(gaussdb_env, table_name):
    from common.doc_store.doc_store_base import MatchTextExpr
    from rag.utils.gaussdb_conn import GaussDBConnection
...
def test_tc_ret_902_vector_topk_positive_negative_and_valid(gaussdb_env, table_name, gaussdb_admin_conn):
    ...
    try:
        ...
        if gaussdb_env["variant_evidence"].get("variant") == "distributed":
            ...
            with gaussdb_admin_conn.cursor() as cur:
                cur.execute(
                    sql.SQL("SELECT id, q_1024_vec <+> %s::floatvector(1024) AS distance FROM {} WHERE kb_id = %s AND id IN ('near', 'mid', 'far') ORDER BY distance, id").format(
                        sql.Identifier(gaussdb_env["schema"], table)
                    ),
...
def test_tc_ret_905_declared_vector_dimensions_have_real_search_support(
    ...
    for dim in dims:
        ...
        try:
            ...
            with gaussdb_admin_conn.cursor() as cur:
                cur.execute(
                    sql.SQL("SELECT id, {} <+> %s::floatvector({}) AS distance FROM {} WHERE kb_id = %s AND id IN ('near', 'mid', 'far') ORDER BY distance, id").format(
