...
def test_centralized_explain_records_fulltext_and_vector_plans(
    ...
    fulltext_plan = _explain(
        gaussdb_admin_conn,
        sql.SQL("SELECT id FROM {} WHERE to_tsvector('simple', coalesce(title_tks, ' ') || ' ' || coalesce(content_sm_ltks, ' ')) @@ plainto_tsquery('simple', %s)").format(
    ...
    vector_plan = _explain(
        gaussdb_admin_conn,
        sql.SQL("SELECT id FROM {} WHERE q_4_vec_valid = TRUE ORDER BY q_4_vec <+> %s::floatvector(4) LIMIT 1").format(sql.Identifier(gaussdb_env["schema"], table)),
        ["[0.1,0.2,0.3,0.4]"],
    )
    ...
    assert "ugin" in fulltext_plan_lower or "bitmap" in fulltext_plan_lower or "to_tsvector" in fulltext_plan_lower
