...
def _insert_chunk(admin_conn, schema, table, *, chunk_id, kb_id, doc_id, content, vector):
    with admin_conn.cursor() as cur:
        cur.execute(
            sql.SQL(
                "INSERT INTO {} (id, kb_id, doc_id, content_with_weight, q_1024_vec, q_1024_vec_valid) "
                "VALUES (%s, %s, %s, %s, %s::floatvector(1024), TRUE)"
...
def test_tc_wrt_1001_centralized_create_insert_get_roundtrip(gaussdb_env, table_name, gaussdb_admin_conn):
    ...
    with gaussdb_admin_conn.cursor() as cur:
        cur.execute(
            sql.SQL(
                "SELECT id, kb_id, doc_id, content_with_weight, q_1024_vec, q_1024_vec_valid "
...
def test_tc_wrt_1010_integration_batch_insert_rolls_back_without_residue(gaussdb_env, table_name, gaussdb_admin_conn):
    ...
    with gaussdb_admin_conn.cursor() as cur:
        ...
        cur.execute(
            sql.SQL("INSERT INTO {} (id, kb_id, doc_id, content_with_weight) VALUES (%s, %s, %s, %s)").format(
                sql.Identifier(gaussdb_env["schema"], table)
            ),
    ...
    with gaussdb_admin_conn.cursor() as cur:
        cur.execute(
            sql.SQL("SELECT id, kb_id, doc_id, content_with_weight FROM {} ORDER BY id").format(
                sql.Identifier(gaussdb_env["schema"], table)
            )
        )
        rows = cur.fetchall()
...
def test_tc_wrt_1007_centralized_reparse_replaces_old_chunks(
    ...
    assert [(row["id"], row["doc_id"], row["content_with_weight"]) for row in _search_chunks(conn, table, kb_id, {"doc_id": "d1"})] == [
        ("c1", "d1", "old chunk")
    ]
    ...
    with gaussdb_admin_conn.cursor() as cur:
        ...
        cur.execute(
            sql.SQL("SELECT id, doc_id, content_with_weight, q_1024_vec, q_1024_vec_valid FROM {} WHERE doc_id = %s").format(
                sql.Identifier(gaussdb_env["schema"], table)
            ),
            ["d1"],
        )
...
def test_tc_ret_101_centralized_simple_parser_recalls_only_matching_chunk(
    ...
    try:
        with gaussdb_admin_conn.cursor() as cur:
            cur.executemany(
                sql.SQL(
                    "INSERT INTO {} (id, kb_id, doc_id, content_with_weight, content_ltks, content_sm_ltks) "
                    "VALUES (%s, %s, %s, %s, %s, %s)"
                ).format(sql.Identifier(gaussdb_env["schema"], table)),
                [
        ...
        with gaussdb_admin_conn.cursor() as cur:
            cur.execute(
                sql.SQL("SELECT id, content_ltks FROM {} ORDER BY id").format(
                    sql.Identifier(gaussdb_env["schema"], table)
                )
            )
