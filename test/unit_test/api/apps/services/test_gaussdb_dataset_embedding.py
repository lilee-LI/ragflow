...
def _load_dataset_module(monkeypatch):
    ...
    _install_module(monkeypatch, "api.db.services.tenant_model_service", TenantModelService=SimpleNamespace())
    ...
    _install_module(monkeypatch, "rag.advanced_rag")
    _install_module(monkeypatch, "rag.advanced_rag.knowlege_compile")
    _install_module(monkeypatch, "rag.advanced_rag.knowlege_compile.wiki", WIKI_PAGE_COMPILE_KWD="wiki")
...
def test_delete_tags_propagates_docstore_update_failure(monkeypatch):
    module = _load_dataset_module(monkeypatch)
    module.settings.docStoreConn.update_result = False

    ok, message = module.delete_tags("kb1", "tenant1", ["risk", "audit"])

    assert ok is False
    assert message == "Failed to update dataset tags in document store"
    assert len(module.settings.docStoreConn.updates) == 1


def test_rename_tag_propagates_docstore_update_failure(monkeypatch):
    module = _load_dataset_module(monkeypatch)
    module.settings.docStoreConn.update_result = False

    ok, message = module.rename_tag("kb1", "tenant1", "old", "new")
