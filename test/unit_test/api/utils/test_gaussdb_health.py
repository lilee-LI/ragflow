...
class FailingGaussDBConnection:
    def health(self):
        raise RuntimeError(
            f"FATAL: password authentication failed password={CFG614_PASSWORD_SENTINEL} "
            f"dsn={FULL_DSN_SENTINEL} access_token={ACCESS_TOKEN_SENTINEL}"
        )
...
def test_tc_cfg_614_get_gaussdb_status_error_masks_password(monkeypatch):
    health_utils = _import_health_utils(monkeypatch)
    monkeypatch.setenv("DOC_ENGINE", "gaussdb")
