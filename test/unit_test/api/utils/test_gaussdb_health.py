...
class LeakyPerformanceGaussDBConnection(FakeGaussDBConnection):
    def health(self):
        return {"status": "healthy", "uri": "sqlbuilder@db.example:19995/postgres?schema=public"}

    def get_performance_metrics(self):
        return {"connection": "disconnected", "error": "password=fake-unit-password"}
...
def test_check_gaussdb_health_healthy(monkeypatch):
    health_utils = _import_health_utils(monkeypatch)
    monkeypatch.setenv("DOC_ENGINE", "gaussdb")
    monkeypatch.setattr(health_utils, "GaussDBConnection", FakeGaussDBConnection)
...
def test_check_gaussdb_health_unhealthy_for_incompatible_mode(monkeypatch):
    health_utils = _import_health_utils(monkeypatch)
    monkeypatch.setenv("DOC_ENGINE", "gaussdb")
    monkeypatch.setattr(health_utils, "GaussDBConnection", IncompatibleGaussDBConnection)
...
def test_check_gaussdb_health_masks_nested_performance_errors(monkeypatch):
    health_utils = _import_health_utils(monkeypatch)
    monkeypatch.setenv("DOC_ENGINE", "gaussdb")
    monkeypatch.setattr(health_utils, "GaussDBConnection", LeakyPerformanceGaussDBConnection)

    result = health_utils.check_gaussdb_health()

    assert result["status"] == "unhealthy"
    assert "fake-unit-password" not in result["details"]["error"]
    assert "password=***" in result["details"]["error"]
