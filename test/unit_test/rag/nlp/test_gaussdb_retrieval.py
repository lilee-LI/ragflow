...
def test_dealer_passes_tokenized_chinese_keywords_to_gaussdb_search(dealer_cls):
    dealer = make_dealer(dealer_cls)
    ...
    expected = ["测试", "检索", "问题"]
    assert kwargs["keywords"] == expected
    assert kwargs["gaussdb_search_params"]["keywords"] == expected
