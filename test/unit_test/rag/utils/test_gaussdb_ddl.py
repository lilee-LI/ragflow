...
def test_regular_index_ddls_keep_long_tenant_indexes_under_gaussdb_limit():
    ...
    for statement in statements:
        index_name = statement.split("CREATE INDEX IF NOT EXISTS ", 1)[1].split(" ON ", 1)[0]
...
def test_distributed_rejects_vector_dimensions_above_1024():
    builder = GaussDBDDLBuilder(schema="public", distributed=True)
    with pytest.raises(ValueError, match="Distributed"):
        builder.build_vector_column_ddls("ragflow_tenant_a", 1536)
...
def test_diskann_index_keeps_long_tenant_index_under_gaussdb_limit():
    ...
    index_name = ddl.split("CREATE INDEX IF NOT EXISTS ", 1)[1].split(" ON ", 1)[0]
    assert len(index_name) <= 63
...
def test_ustore_check_sql_uses_bound_schema_and_table_params():
    builder = GaussDBDDLBuilder(schema="public")
    sql, params = builder.build_ustore_check_sql("ragflow_tenant_a")

    assert "pg_class" in sql
    assert "reloptions" in sql
    assert params == ["public", "ragflow_tenant_a"]
