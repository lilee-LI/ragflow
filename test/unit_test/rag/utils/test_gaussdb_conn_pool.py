...
def test_tc_cfg_301_pool_initializes_driver_with_complete_connection_options(monkeypatch):
    ...
    assert created["kwargs"] == {
        ...
        "user": "u",
        "password": "p",
        "options": "-c search_path=zlw -c default_transaction_read_only=off",
    }
...
def test_tc_cfg_310_gaussdb_config_and_pool_do_not_read_ssl_timeout_or_pool_tuning(monkeypatch):
    ...
    assert created["kwargs"]["options"] == "-c search_path=public -c default_transaction_read_only=off"
...
def _assert_schema_privilege_query(sql, params, user, schema):
    assert " ".join(sql.split()) == (
        "SELECT has_schema_privilege(%s, %s, %s) AS has_usage, "
...
def test_tc_cfg_315_pool_get_conn_discards_closed_connection_and_retries_once():
    class ClosedFirstLegacyPool(SequencedPool):
        def __init__(self):
            super().__init__()
    ...
    cfg = GaussDBConfig("db.example", 19995, "postgres", "sqlbuilder", "fake-unit-password", "ragflow_gaussdb_docengine_it")
    fake_pool = ClosedFirstLegacyPool()
    pool = GaussDBConnectionPool(cfg, pool=fake_pool)

    conn = pool.get_conn()

    assert conn is fake_pool.live_conn
    assert fake_pool.returned == [fake_pool.dead_conn]
