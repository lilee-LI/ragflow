...
def test_tc_ret_109_chinese_ngram_query_preserves_english_semantics_and_highlight_text():
    ...
    assert conn.get_highlight(result, ["上海", "RAGFlow", "database"], "content_with_weight") == {
        "mixed": "<em>上海</em> <em>RAGFlow</em> <em>database</em> 审计"
    }
    assert conn.get_highlight(result, ["深圳数据", "深圳"], "content_with_weight") == {
        "overlap": "<em>深圳数据</em>库审计"
    }
...
def test_tc_ret_605_fulltext_search_projects_parameterized_highlight_column():
    ...
    assert (
        "ts_headline('simple', COALESCE(content_with_weight, ' '), plainto_tsquery('simple', %s), "
...
def test_tc_ret_601_aggregation_sql_groups_field_values():
    ...
    assert sql == (
        'SELECT docnm_kwd AS value, COUNT(1) AS count FROM "public"."ragflow_tenant" '
        "WHERE kb_id = %s AND docnm_kwd IS NOT NULL "
        "GROUP BY value ORDER BY count DESC, value ASC LIMIT %s"
    )
    assert params == ["kb1", 1000]


def test_tc_ret_602_aggregation_sql_rejects_jsonb_array_fields():
    builder = GaussDBSearchBuilder(schema="public")
    ...
    assert str(exc_info.value) == "JSONB array aggregation is not supported by GaussDB A mode"
...
def test_tc_ret_108_filter_only_search_parameterizes_kb_limit_and_offset():
    ...
    assert sql == (
        "SELECT id, kb_id, docnm_kwd, 0.0 AS _score, COUNT(*) OVER() AS __total "
        'FROM "public"."ragflow_tenant" WHERE kb_id = %s '
        "ORDER BY docnm_kwd DESC LIMIT %s OFFSET %s"
    )
...
def test_regression_compile_kwd_uses_extra_jsonb_mapping():
    builder = GaussDBSearchBuilder(schema="public")
