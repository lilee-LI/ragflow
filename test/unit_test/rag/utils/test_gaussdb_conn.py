...
CANONICAL_CREATE_IDX_1024_EXECUTIONS = [
  ...
  PRIMARY KEY (kb_id, id)
    ...
    (
        'CREATE INDEX IF NOT EXISTS "idx_gdb_ragflow_wrt_012_doc_id" '
        'ON "public"."ragflow_wrt_012" (doc_id)',
        [],
    ),
    (
        'CREATE INDEX IF NOT EXISTS "idx_gdb_ragflow_wrt_012_available_int" '
        'ON "public"."ragflow_wrt_012" (available_int)',
        [],
    ),
    (
        'CREATE INDEX IF NOT EXISTS "idx_gdb_ragflow_wrt_012_knowledge_graph_kwd" '
        'ON "public"."ragflow_wrt_012" (knowledge_graph_kwd)',
        [],
    ),
    (
        'CREATE INDEX IF NOT EXISTS "idx_gdb_ragflow_wrt_012_entity_type_kwd" '
        'ON "public"."ragflow_wrt_012" (entity_type_kwd)',
        [],
    ),
    (
        'CREATE INDEX IF NOT EXISTS "idx_gdb_ragflow_wrt_012_removed_kwd" '
        'ON "public"."ragflow_wrt_012" (removed_kwd)',
        [],
    ),
    (
  ...
  USING ugin(to_tsvector('ngram', """
    ...
    (
        'ALTER TABLE "public"."ragflow_wrt_012" ADD COLUMN IF NOT EXISTS q_1024_vec '
        "floatvector(1024) DEFAULT (array_fill(0, ARRAY[1024])::text::floatvector(1024))",
        [],
    ),
    (
        'ALTER TABLE "public"."ragflow_wrt_012" ADD COLUMN IF NOT EXISTS q_1024_vec_valid '
        "BOOLEAN DEFAULT FALSE NOT NULL",
        [],
    ),
    ("SET LOCAL maintenance_work_mem = '1GB'", []),
    (
        'CREATE INDEX IF NOT EXISTS "idx_gdb_ragflow_wrt_012_q_1024_vec_diskann" '
        'ON "public"."ragflow_wrt_012" USING gsdiskann (q_1024_vec COSINE) WITH (subgraph_count=1)',
        [],
    ),
]
...
CANONICAL_CREATE_DOC_META_EXECUTIONS = [
  ...
  PRIMARY KEY (id)
    ...
    (
        'CREATE INDEX IF NOT EXISTS "idx_gdb_ragflow_doc_meta_tenant_kb_id" '
        'ON "public"."ragflow_doc_meta_tenant" (kb_id)',
        [],
    ),
]
...
def test_tc_wrt_301_insert_doc_meta_uses_parameterized_upsert():
    ...
    expected_sql = (
        'INSERT INTO "public"."ragflow_doc_meta_unit" (id, kb_id, meta_fields) '
        "VALUES (%s, %s, %s::jsonb) ON DUPLICATE KEY UPDATE meta_fields = VALUES(meta_fields)"
    )
...
def test_tc_wrt_309_delete_doc_meta_scopes_by_id_and_kb_id():
    ...
    assert deleted == 1
    assert cursor.executed == [
        ('DELETE FROM "public"."ragflow_doc_meta_tenant" WHERE id = %s AND kb_id = %s', ["doc1", "kb1"])
    ]
...
def test_tc_wrt_614_delete_allows_kb_scope_from_condition_without_dataset_id():
    ...
    assert deleted == 1
    assert cursor.executed == [
        ('DELETE FROM "public"."ragflow_tenant" WHERE id = %s AND kb_id = %s', ["c1", "kb1"])
    ]
    sql = cursor.executed[0][0]
...
def test_tc_wrt_503_update_builds_parameterized_set_clause_and_scopes_kb_id():
    ...
    assert cursor.executed == [
        ('UPDATE "public"."ragflow_tenant" SET available_int = %s WHERE id = %s AND kb_id = %s', [0, "c1", "k1"])
    ]
    sql = cursor.executed[0][0]
    assert "c1" not in sql
...
def test_regression_update_ignores_redundant_matching_id():
    cursor = RecordingCursor()
    conn = make_conn(cursor)
    ...
    assert cursor.executed == [
        (
            'UPDATE "public"."ragflow_tenant" SET content_with_weight = %s, '
            "q_3_vec = %s::floatvector(3), q_3_vec_valid = TRUE WHERE id = %s AND kb_id = %s",
            ["updated", "[0.1,0.2,0.3]", "c1", "kb1"],
        )
    ]


def test_tc_wrt_509_update_rejects_unknown_column_without_sql():
    cursor = RecordingCursor()
    conn = make_conn(cursor)
    ...
    with pytest.raises(ValueError, match="unknown column for update: unknown_col"):
        conn._build_set_clause({"unknown_col": 1}, is_meta=False, condition={"id": "c1"})
    assert cursor.executed == []
...
def test_supplemental_tc_wrt_506_json_search_treats_wildcards_as_literals():
    cursor = RecordingCursor()
    cursor.rowcount = 1
    conn = make_conn(cursor)
...
def test_supplemental_tc_wrt_507_same_column_remove_then_add_uses_one_assignment():
    cursor = RecordingCursor()
    cursor.rowcount = 1
    conn = make_conn(cursor)
...
def test_tc_wrt_505_update_remove_column_sets_null_with_scoped_parameters():
    ...
    assert updated is True
    assert cursor.executed == [
        ('UPDATE "public"."ragflow_tenant" SET important_kwd = NULL WHERE id = %s AND kb_id = %s', ["c1", "k1"])
    ]
    sql = cursor.executed[0][0]
    assert "c1" not in sql
    assert "k1" not in sql
...
def test_tc_wrt_611_delete_jsonb_multivalue_conditions_use_contains_predicates():
    ...
    assert deleted == 2
    assert cursor.executed == [
        ('DELETE FROM "public"."ragflow_tenant" WHERE (tag_kwd @> %s::jsonb) AND kb_id = %s', ['["t1"]', "k1"])
    ]
    sql = cursor.executed[0][0]
    assert "t1" not in sql
    assert "k1" not in sql
...
def test_tc_wrt_607_delete_must_not_exists_uses_is_null_with_kb_scope():
    ...
    assert deleted == 1
    assert cursor.executed == [
        ('DELETE FROM "public"."ragflow_tenant" WHERE tag_kwd IS NULL AND kb_id = %s', ["k1"])
    ]


def test_regression_delete_compile_kwd_uses_extra_jsonb_mapping():
    cursor = RecordingCursor()
    cursor.rowcount = 2
    conn = make_conn(cursor)
    ...
    assert cursor.executed == [
        (
            'DELETE FROM "public"."ragflow_tenant" WHERE doc_id = %s AND id IN (%s, %s) '
            "AND (extra #>> '{compile_kwd}') IS NULL AND kb_id = %s",
            ["d1", "c1", "c2", "k1"],
        )
    ]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"unknown": "x"}, "ragflow_tenant", False, "unsupported filter column: unknown"),
        ({"unknown": "x"}, "ragflow_doc_meta_tenant", True, "unsupported metadata filter column: unknown"),
    ],
    ids=["empty-scalar-list", "empty-jsonb-list", "chunk-column", "metadata-column"],
)
...
def test_tc_wrt_010_create_idx_rolls_back_when_diskann_ddl_fails():
...
def test_tc_wrt_012_create_idx_executes_complete_ddl_sequence():
    ...
    assert result is True
    assert len(CANONICAL_CREATE_IDX_1024_EXECUTIONS) == 13
    assert cursor.executed == CANONICAL_CREATE_IDX_1024_EXECUTIONS
...
def test_tc_ret_009_search_empty_deep_page_falls_back_to_total_count():
    ...
    assert cursor.executed[1] == (
        'SELECT id, kb_id, 0.0 AS _score, COUNT(*) OVER() AS __total '
        'FROM "public"."ragflow_tenant" WHERE kb_id IN (%s) '
        'ORDER BY kb_id ASC, id ASC LIMIT %s OFFSET %s',
        ["kb1", 1, 0],
    )
...
def test_regression_empty_doc_ids_keep_kb_scope():
...
def test_tc_ret_601_search_executes_single_field_aggregation():
    ...
    assert cursor.executed == [
        (
            'SELECT doc_type_kwd AS value, COUNT(1) AS count FROM "public"."ragflow_tenant" '
            'WHERE kb_id IN (%s) AND doc_type_kwd IS NOT NULL '
...
def test_tc_mgf_811_fetch_metadata_doc_ids_preserves_probe_limit_and_row_shapes():
    ...
    assert cursor.executed == [
        (
            'SELECT id FROM "public"."ragflow_doc_meta_tenant" '
            "WHERE kb_id IN (%s) AND (meta_fields ? 'a') ORDER BY id LIMIT %s",
            ["kb1", 4],
        )
    ]
    assert cursor.closed is True
...
def test_tc_ret_607_search_chunk_aggregation_returns_empty_for_jsonb_array_fields():
    cursor = RecordingCursor()
    conn = make_conn(cursor)
...
def test_tc_wrt_504_update_overwrites_metadata_and_derives_title_with_scoped_parameters():
    ...
    assert cursor.executed == [
        (
            'UPDATE "public"."ragflow_tenant" '
            "SET metadata = %s::jsonb, docnm_kwd = %s WHERE id = %s AND kb_id = %s",
...
def test_tc_wrt_608_delete_exists_stays_within_kb_scope():
    ...
    assert deleted == 2
    assert cursor.executed == [
        ('DELETE FROM "public"."ragflow_tenant" WHERE doc_id IS NOT NULL AND kb_id = %s', ["kb1"])
    ]
...
def test_regression_parse_match_tokenizes_original_query(monkeypatch):
    queries = []
