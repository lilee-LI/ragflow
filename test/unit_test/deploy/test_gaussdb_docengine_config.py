...
def test_docker_env_lists_gaussdb_as_doc_engine():
    ...
    assert re.search(r"^GAUSSDB_HOST=$", env_text, re.MULTILINE)
    assert re.search(r"^GAUSSDB_PORT=$", env_text, re.MULTILINE)
    assert re.search(r"^GAUSSDB_DATABASE=.+$", env_text, re.MULTILINE)
    assert re.search(r"^GAUSSDB_USER=$", env_text, re.MULTILINE)
    assert re.search(r"^GAUSSDB_PASSWORD=$", env_text, re.MULTILINE)
    assert re.search(r"^GAUSSDB_SCHEMA=$", env_text, re.MULTILINE)
    for key in (
        "GAUSSDB_HOST",
        "GAUSSDB_PORT",
        "GAUSSDB_DATABASE",
