...
def _helm_env_secret(documents):
    matches = [
        ...
        if document.get("kind") == "Secret" and document.get("metadata", {}).get("name", "").endswith("-env-config")
    ]
...
def test_tc_cfg_804_docker_rendered_gaussdb_config_excludes_tuning_fields(tmp_path):
    ...
    assert "password: '${GAUSSDB_PASSWORD}'" in gaussdb_block
    assert set(config) == {"host", "port", "database", "user", "password", "schema"}
...
def test_tc_cfg_805_helm_template_renders_complete_gaussdb_secret():
    ...
    assert {key: string_data[key] for key in (
        "DOC_ENGINE",
        "GAUSSDB_HOST",
        "GAUSSDB_PORT",
        "GAUSSDB_DATABASE",
...
def test_tc_cfg_806_helm_render_creates_no_gaussdb_workload_or_service():
    ...
    assert result.returncode == 0, result.stderr
    documents = _load_yaml_documents(result.stdout)
    orchestrated_documents = [
        document
        for document in documents
        if document.get("kind")
        in {"Service", "Pod", "Deployment", "StatefulSet", "DaemonSet", "Job", "CronJob", "ReplicaSet"}
    ]
    gaussdb_references = [
